...
export default class SonosDevice extends SonosDeviceBase {
  ...
  public async PlayNotification(options: PlayNotificationOptions): Promise<boolean> {
    ...
    const originalState = this.CurrentTransportStateSimple ?? (await this.AVTransportService.GetTransportInfo()).CurrentTransportState as TransportState;
    this.debug('Current state is %s', originalState);

    // Original data to revert to
    const originalVolume = this.Volume ?? (await this.RenderingControlService.GetVolume({ InstanceID: 0, Channel: 'Master' })).CurrentVolume;
    ...
    const shouldRevert = await this.PlayNextNotification(originalState, originalVolume);
   ...
   * @param {number} [options.timeout] Number of seconds the notification should play, as a fallback if the event doesn't come through.
   * @param {number} [options.volume] Change the volume for the notication and revert afterwards.
   * @returns {Promise<boolean>} returns true if notification actually played or queued
   * @memberof SonosDevice
   */
  ...
  private async PlayNextNotification(originalState: TransportState, originalVolume: number, havePlayed?: boolean): Promise<boolean> {
    ...
    if (notification.onlyWhenPlaying === true && !(originalState === TransportState.Playing || originalState === TransportState.Transitioning)) {
      ...
      if (notification.volume !== undefined && notification.volume !== originalVolume) {
        await this.RenderingControlService.SetVolume({ InstanceID: 0, Channel: 'Master', DesiredVolume: notification.volume });
    ...
    return this.PlayNextNotification(originalState, originalVolume, result);
  }
